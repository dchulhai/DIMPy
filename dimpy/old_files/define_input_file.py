import re
from .constants import NOCONV, ANGSTROM2BOHR, elem
from .dim_reader import DIMReader, SUPPRESS


def define_input_file():
    """Defines what to read from the input file."""

    # Create a InputReader class.  Valid comment characters are :: and #
    reader = DIMReader(default=SUPPRESS, comment=['::', '#'])
    ...
    reader.add_line_key('algorithm', type=(0, 1, 2, 3,
                                           'direct', 'brute',
                                           'single', 'multi'), default=-1)
    ...
    unitglob = {'len': '?',
                ...
                'default': 'hartree'}
    for el in elem:  # Elements defined from constants module
